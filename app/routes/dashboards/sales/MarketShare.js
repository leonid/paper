            ...
            <Svg style="width:100%;height:250px">
                ...
                <Chart margin="20 20 50 180" axes={{
                    ...
                    <Repeater records:bind="$page.marketShare">
                        <Bar y:bind="$record.company" x:bind="$record.share"
                             style={{"fill": {expr: "{$record.highlight} ? '#666' : '#bbb'"}}} size={0.4}/>
                        <Bar y:bind="$record.company" x0:bind="$record.lastMonthShare" x:bind="$record.lastMonthShare"
                             style="stroke:#666;stroke-width:1px" size={0.6} name="1 month ago" legendShape="vline"/>
                    </Repeater>
                </Chart>
            </Svg>
        </div>
