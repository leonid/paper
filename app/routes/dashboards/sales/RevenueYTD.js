                ...
                <Chart margin="20 0 50 0" axes={{
                    ...
                    <Repeater records:bind="$page.revenueYTD">
                        <Range y1:bind="$record.region" y2:bind="$record.region" ySize={1} hidden>
                            ...
                            <Marker visible:expr="{$record.revenue} < {$record.target}" anchors="0.5 0 0.5 0"
                                    offset="0 215 0 215" style="fill:red;stroke:none" size="10"/>
                    ...
                    <Text anchors="1 1 1 1" offset="5 0 0 0" textAnchor="end" dy="1em"
                          value={computable("$page.revenueYTD", rev => Format.value(rev.reduce((a, b) => a + b.revenue, 0), 'n'))}/>
                </Chart>
            </Svg>
        </div>
    </Legend.Scope>
