                ...
                <Chart margin="20 0 50 0" axes={{
                    ...
                    <Repeater records:bind="$page.revenueQTD">
                        <Range y1:bind="$record.region" y2:bind="$record.region" ySize={1} hidden>
                            <Marker visible:expr="{$record.revenue} + {$record.pipeline90} < {$record.target}"
                                    anchors="0.5 0 0.5 0" offset="0 15 0 15" style="fill:red;stroke:none" size="10"/>
                        ...
                        <Bar y:bind="$record.region" x0:bind="$record.revenue"
                             x:expr="{$record.revenue}+{$record.pipeline75}" style="fill:#eee" size={0.4} name="75%"/>
                        <Bar y:bind="$record.region" x0:bind="$record.revenue"
                             x:expr="{$record.revenue}+{$record.pipeline90}" style="fill:#ddd" size={0.4} name="90%"/>
                        <Bar y:bind="$record.region" x0:bind="$record.target" x:bind="$record.target"
                    ...
                    <Text anchors="1 1 1 1" offset="5 0 0 0" textAnchor="end" dy="1em"
                          value={computable("$page.revenueQTD", rev => Format.value(rev.reduce((a, b) => a + b.revenue, 0), 'n'))}/>
                </Chart>
            </Svg>
        </div>
    </Legend.Scope>
