                ...
                <Chart margin="20 0 50 0" padding="0 270 0 0" axes={{
                    ...
                    <Repeater records:bind="$page.productSalesYTD">
                        <Range y1:bind="$record.product" y2:bind="$record.product" ySize={1} hidden>
                            ...
                            <Marker visible:expr="{$record.revenue} < {$record.target}" anchors="0.5 0.34 0.5 0.34"
                                    offset="0 15 0 15" style="fill:red;stroke:none" size="10"/>
                            <Text value:bind="$record.product" anchors="0.5 0.34 0.5 0.34" offset="0 0 0 0" dy="0.4em"
                                  dx="30px"/>
                            <Text value:tpl="{$record.revenue:n}" anchors="0.5 1 0.5 1" dx="270px" dy="0.4em"
                                  textAnchor="end"/>
                        </Range>
                        <Bar y:bind="$record.product" x:bind="$record.revenue" xAxis="revenue" style="fill:#666"
                             size={0.4} name="Revenue"/>
                        <Bar y:bind="$record.product" x0:bind="$record.target" x:bind="$record.target" xAxis="revenue"
                             style="stroke:black;stroke-width:1" size={0.6}/>
                        <Bar y:bind="$record.product" x:bind="$record.units" xAxis="units" style="fill:#666" size={0.4}
                             name="Revenue"/>
                        <Bar y:bind="$record.product" x0:bind="$record.unitsTarget" x:bind="$record.unitsTarget"
                             xAxis="units" style="stroke:black;stroke-width:1" size={0.6}/>
                    </Repeater>
                    ...
                    <Text anchors="1 1 1 1" offset="5 270 0 270" textAnchor="end" dy="1em"
                          value={computable("$page.productSalesYTD", rev => Format.value(rev.reduce((a, b) => a + b.revenue, 0), 'n'))}/>
                </Chart>
            </Svg>
        </div>
    </Legend.Scope>
